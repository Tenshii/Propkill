	...
	if PK.DamageToRender and #PK.DamageToRender > 0 then
		for k, v in pairs( PK.DamageToRender ) do
			...
			if v.Alpha > 0 then
	           ...
	           surface.CreateFont("coolvetica", 35, 400, false, false, "DamageIndicFont6")
...
function GM:DrawPropOwner()
    ...
    if tr.HitNonWorld  then
        if tr.Entity:IsValid() and not tr.Entity:IsPlayer() and not LocalPlayer():InVehicle() and PK.SeenProps[ tr.Entity ] then
        	local owner = PK.SeenProps[ tr.Entity ]:Nick() or "Disconnected Player"
        	if owner then
	            local PropOwner = "Owner: " .. owner
...
function GM:DrawHealthBar()
	...
	if LocalPlayer():GetNWBool( "IsSpectating", false ) then Health = 0 end
...
function GM:DrawKDBar()
	local SKills, SDeaths = LocalPlayer().SKills or 1, LocalPlayer().SDeaths or 1

	if Target and Target:IsPlayer() then
		SKills = Target.SKills or 1
		SDeaths = Target.SDeaths or 1
	end
	...
	if fighting then
		local fighters = glon.decode( GetGlobalString( "Fighters" ) )
		draw.SimpleText( "Fighter1: " .. fighters[1]:Nick() .. " (" .. fighters[2]:Deaths() .. ")", "UiBold", 48, ScrH() - 47, WHITECOLOUR, 0, 1)
		draw.SimpleText( "Fighter2: " .. fighters[2]:Nick() .. " (" .. fighters[1]:Deaths() .. ")", "UiBold", 48, ScrH() - 35, WHITECOLOUR, 0, 1)
	else
